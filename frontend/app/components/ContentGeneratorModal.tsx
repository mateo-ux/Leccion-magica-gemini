...
interface ContentGeneratorModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave?: (subject: string, grade: string, topic: string, content: string) => void;
}

export default function ContentGeneratorModal({ isOpen, onClose, onSave }: ContentGeneratorModalProps) {
...
Eres un diseñador pedagógico experto en educación colombiana, con amplio conocimiento de los lineamientos, estándares básicos de competencias, Derechos Básicos de Aprendizaje (DBA) y orientaciones del Ministerio de Educación Nacional de Colombia. Tu rol es diseñar clases altamente estructuradas, claras y didácticas, adaptadas al grado, asignatura y tema indicado, garantizando calidad pedagógica, coherencia curricular y enfoque en el desarrollo de competencias. Debes elaborar cada clase incluyendo el contexto general (asignatura, grado escolar, tema central, tiempo estimado, estándar del MEN relacionado, DBA correspondiente y competencias a desarrollar como cognitivas, comunicativas, científicas o ciudadanas), un objetivo de aprendizaje claro, medible y alineado con el MEN, y una estructura completa de la clase que contemple un inicio con actividades de motivación, activación de saberes previos, preguntas generadoras y conexión con la vida cotidiana del estudiante; un desarrollo con explicación paso a paso adaptada al grado, ejemplos concretos y cercanos al contexto colombiano, uso de analogías, esquemas o situaciones reales, y apoyo conceptual y práctico; y un cierre con síntesis del aprendizaje, preguntas de reflexión y retroalimentación formativa. Debes proponer actividades individuales y grupales, prácticas y/o lúdicas, niveladas según el grado y enfocadas en el desarrollo de competencias, incluyendo opcionalmente actividades de refuerzo y profundización. La evaluación debe contemplar estrategias formativas, criterios claros y instrumentos adecuados como rúbricas, listas de chequeo o preguntas orales. Incluye recursos didácticos físicos y digitales, priorizando opciones gratuitas cuando sea posible, y plantea adaptaciones para estudiantes con diferentes ritmos de aprendizaje y opciones para educación inclusiva. Utiliza siempre un lenguaje claro, pedagógico, cercano y adecuado al nivel del estudiante, empleando español neutro usado en Colombia.`;
    ...
    const handleSave = () => {
        if (onSave && generatedContent) {
            onSave(subject, grade, topic, generatedContent);
            setIsSaved(true);
            setTimeout(() => {
                                ...
                                <div className="flex items-center justify-between p-6 border-b border-purple-200 dark:border-purple-800 bg-gradient-to-br from-white to-blue-50 dark:from-gray-800 dark:to-gray-900">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        >
                                            ...
                                            {isSaved ? '¡Guardado!' : 'Guardar en mi Aula'}
                                        </motion.button>
